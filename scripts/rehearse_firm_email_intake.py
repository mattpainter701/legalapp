...
async def main():
    ...
    try:
        ...
        for tenant in (first, second):
            aliases.append(await conn.fetchval("""INSERT INTO inbound_email_aliases
                (tenant_id,kind,token_hash,encrypted_local_part) VALUES ($1,'firm',$2,'fixture') RETURNING id""",
                tenant, uuid.uuid4().hex * 2))
        for statement, error in [
            ("INSERT INTO inbound_email_aliases (tenant_id,kind,token_hash,encrypted_local_part) VALUES ($1,'firm',$2,'fixture')", asyncpg.UniqueViolationError),
            ("INSERT INTO inbound_email_aliases (tenant_id,kind,token_hash,encrypted_local_part) VALUES ($1,'matter',$2,'fixture')", asyncpg.CheckViolationError),
        ]:
            try:
        ...
        for tenant, alias in zip((first, second), aliases):
            await conn.execute("""INSERT INTO inbound_emails
                (tenant_id,alias_id,envelope_sender,recipient,subject,message_sha256,raw_size,occurred_at)
        ...
        await conn.execute("GRANT SELECT, UPDATE ON inbound_email_aliases, inbound_emails TO firm_intake_rehearsal")
        ...
        assert await conn.fetchval("SELECT count(*) FROM inbound_emails WHERE alias_id=ANY($1::uuid[])", aliases) == 1
        assert await conn.fetchval("SELECT count(*) FROM inbound_email_aliases WHERE id=ANY($1::uuid[])", aliases) == 1
        ...
        assert await conn.fetchval("SELECT count(*) FROM inbound_email_aliases WHERE id=ANY($1::uuid[])", aliases) == 2
        assert await conn.fetchval("SELECT count(*) FROM inbound_emails WHERE alias_id=ANY($1::uuid[])", aliases) == 1
