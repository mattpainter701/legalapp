...
- Uses the existing HTTP-only WellPled session. When absent, NAA obtains an Entra access token only long enough to exchange it for HTTP-only WellPled cookies. No WellPled credential is stored in browser storage or a URL.

## Local setup

1. Copy `.env.example` to `.env.local` and configure the Entra app ID and delegated WellPled API scope.
...
The server foundation is feature-gated with `OFFICE_ASSISTANT_ENABLED=false` by default. Before enabling it, apply migration `095_office_assistant`, then configure `OFFICE_ENTRA_CLIENT_ID`, `OFFICE_ENTRA_API_AUDIENCE`, and `OFFICE_ENTRA_REQUIRED_SCOPE`. The exchange endpoint never creates users; each person must already have a linked Microsoft WellPled identity.
