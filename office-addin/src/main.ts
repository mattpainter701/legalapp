...
async function report(result: ActionExecutionResult): Promise<void> {
  try {
    ...
    setStatus('The Office change completed, but Clarity could not record its audit result.', true)
  }
}
...
async function capture(): Promise<void> {
  ...
  refreshProposeState()
  setStatus('Context captured. Review the scope above before asking Clarity.')
}
...
Office.onReady(async (info) => {
  ...
  if (!adapter) {
    element('unsupported-detail').textContent = `Clarity cannot work in ${info.host || 'this Office application'}. Open it in Word, Excel, or Outlook.`
  ...
  try {
    setStatus('Checking your Clarity session…')
    const user = await session.ensure(capabilities.requirementSets.NestedAppAuth_1_1 === true)
    authenticated = true
    setStatus(user.full_name ? `Signed in as ${user.full_name}.` : 'Signed in to Clarity Legal.')
  } catch (error) {
    authenticated = false
