    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute(
            ...
            ON CONFLICT (source_key, external_id) DO UPDATE SET
                ...
                text_content=EXCLUDED.text_content, metadata=legal_documents.metadata || EXCLUDED.metadata,
                updated_at=now() RETURNING id
            """,
            [document.source_key, document.external_id, document.document_type, document.title,
             document.citation, document.jurisdiction, document.authority_tier,
             document.document_status, document.publication_date, document.effective_date,
             document.canonical_url, document.source_modified_at, document.retrieved_at,
             document.content_hash, document.raw_media_type, document.parser_version,
             document.text, json.dumps(document.metadata)],
        )
            ...
            for index, content in enumerate(chunk_text(document.text)):
                cursor.execute(
                    ...
                    [document_id, index, content,
                     hashlib.sha256(content.encode("utf-8")).hexdigest(), json.dumps(document.metadata)],
                )
                chunks_created += 1
    return {"external_id": document.external_id, "changed": changed, "chunks_created": chunks_created}


def refresh_source_status(conn: Any, source_keys: set[str] | list[str] | tuple[str, ...]) -> None:
