...
This UX is layered on the landed Firm Memory source-authorization foundation. Do not infer unified rollout from the user's broad `search_firm_memory` entitlement, the legacy matter-only endpoint, or an existing SMB matter binding.
