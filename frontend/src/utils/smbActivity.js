...
export function buildSmbOperationalActivity({ agents = [], shares = [], credentials = [], updates = {}, accessEntries = [] } = {}) {
  ...
  agents.forEach((agent) => {
    ...
    add({ id: `agent-heartbeat-${agent.id}`, kind: 'agent', title: `${name} heartbeat`, detail: `${statusLabel(agent.status)} · ${agent.agent_version || 'version unknown'}`, state: agent.status === 'active' ? 'success' : 'warning', occurredAt: agent.last_heartbeat || agent.updated_at })
