...
export default function WordDeriveDraftAction({ templateId, fields = [], sourceReview = {}, reviewedSchema = {}, suggestedMode = 'prose', onCreated }) {
  ...
  const mode = overrideMode ?? suggestedMode
  useEffect(() => setOverrideMode(null), [templateId])
  ...
  return <section className="rounded border border-brand-line p-3" aria-label="Create derived Word draft">
    ...
    <label className="mt-2 block text-xs">Source type<select aria-label="Word source type" value={mode} disabled={busy} onChange={event => setOverrideMode(event.target.value)} className="mt-1 block rounded border p-2 text-sm"><option value="prose">Prose</option><option value="form">Form</option></select></label>
