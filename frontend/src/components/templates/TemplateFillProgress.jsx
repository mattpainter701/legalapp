export default function TemplateFillProgress({ progress, filter, onFilter, onNext }) {
  return <section aria-label="Document completion" className="rounded-lg border border-brand-line bg-brand-bg p-3">
    <div className="flex flex-wrap items-center justify-between gap-2">
      ...
      <button type="button" onClick={onNext} disabled={!progress.remaining.length && !progress.review.length} className="rounded border border-brand-line px-3 py-1.5 text-xs font-semibold disabled:opacity-40">Next field needing attention</button>
    </div>
