...
export default function WordImportWorkspace({ file, analysis, fields, onFieldsChange, onAddField }) {
  ...
  const needsReview = included.filter(field => field.review_required || field.ai_suggested || Number(field.confidence ?? 1) < 0.75)
  ...
  const update = changes => onFieldsChange(fields.map((field, index) => index === active?.index ? { ...field, ...changes } : field))
  const pickText = () => {
      ...
      <aside className="border-t border-brand-line p-4 lg:border-l lg:border-t-0" aria-label="Detected Word fields">
        ...
        <ul className="mt-2 max-h-64 space-y-2 overflow-y-auto">
          {entries.map(entry => <li key={entry.identity}><button type="button" aria-label={`Select ${entry.field.label || entry.field.name}`} aria-pressed={entry.identity === active?.identity} onClick={() => setSelected(entry.identity)} className="w-full rounded border border-brand-line p-2 text-left text-sm aria-pressed:border-brand-accent aria-pressed:bg-brand-accent/10">
            ...
            <span className="text-xs">{entry.field.included === false ? 'Excluded' : entry.field.review_required || entry.field.ai_suggested || Number(entry.field.confidence ?? 1) < 0.75 ? 'Needs review' : 'Detected · verify replacement'}</span>
          </button></li>)}
        </ul>
