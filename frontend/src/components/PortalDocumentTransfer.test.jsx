...
vi.mock('../api', () => ({ getClientPortalUploadLink: vi.fn(), sendClientPortalMessage: vi.fn(), uploadClientPortalDocument: vi.fn() }))
afterEach(cleanup)
beforeEach(() => {
  vi.resetAllMocks()
...
it('uploads every selected file, keeps source folders and retries only failures', async () => {
  ...
  fireEvent.change(screen.getByLabelText('Choose folder'), { target: { files: [first, second] } })
  await user.type(screen.getByLabelText('Description (optional)'), 'Transfer')
  await user.click(screen.getByRole('button', { name: 'Upload remaining files / retry failures' }))
  ...
  await user.click(screen.getByRole('button', { name: 'Upload remaining files / retry failures' }))
  await screen.findByText('2 of 2 files uploaded')
...
it('stops the batch on expired portal access', async () => {
  ...
  await user.click(screen.getByRole('button', { name: /Upload remaining/ }))
...
it('shows only the explicitly published upload link and submits a source for review', async () => {
  ...
  await user.type(screen.getByLabelText('Or send a cloud/fileshare link'), 'https://files.example/source')
  await user.click(screen.getByRole('button', { name: 'Send link for import' }))
  ...
  expect(await screen.findByText(/Files have not been imported yet/)).toBeInTheDocument()
...
it('rejects non-HTTPS links and reports message failures', async () => {
  const user = userEvent.setup()
  render(<PortalDocumentTransfer onUploaded={vi.fn()} onSessionError={() => false} />)
  const input = screen.getByLabelText('Or send a cloud/fileshare link')
