...
export default function ContactPicker({ value, onChange, placeholder = 'Search contacts...', className = '' }) {
  ...
  if (selected) {
    return (
      <div className={`flex items-center gap-2 px-3 py-2 border border-brand-line rounded bg-brand-bg-soft ${className}`}>
        ...
        <button onClick={handleClear} className="text-brand-muted hover:text-brand-rose transition-colors">
          <X size={13} />
        </button>
      </div>
    )
  ...
  return (
    <div ref={wrapperRef} className={`relative ${className}`}>
      ...
      {open && (loading || results.length > 0) && (
        <div className="absolute z-50 top-full mt-1 w-full bg-white border border-brand-line rounded shadow-lg max-h-56 overflow-y-auto">
          {loading && (
