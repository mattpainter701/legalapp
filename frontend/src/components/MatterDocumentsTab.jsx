...
export default function MatterDocumentsTab({ matterId, onCloudFolderChange, onReviseDocument }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {previewDocument && <section aria-label="Document preview" className="rounded-xl border border-brand-line bg-brand-surface p-4"><div className="flex justify-between"><strong>{previewDocument.filename}</strong><button type="button" onClick={() => setPreviewDocument(null)}>Close preview</button></div>{previewDocument.content_type === 'application/pdf' ? <GeneratedPdfPreview key={previewDocument.id} title={previewDocument.filename} source={getMatterDocumentDownloadUrl(matterId, previewDocument.id)} /> : <a href={getMatterDocumentDownloadUrl(matterId, previewDocument.id)} className="underline">Open document</a>}</section>}
