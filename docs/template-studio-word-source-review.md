...
Repeated generic bracket placeholders such as `[AMOUNT]`, `[DATE]`, `[VIN NUMBER]` and `[ACCOUNT NUMBER]` become separate fields at their source locations. Review their context and labels. Use **Use the same value as** only when two fields represent the same fact. Repeated specific party aliases and authored `{{variable}}` tokens continue to share their named value. Linked fields must refer directly to an included independent field of the same type; chains, cycles and choice links are rejected.
