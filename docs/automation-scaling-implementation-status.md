...
| W5 | Broad BYO-harness channel contract; per-grant call and per-result byte limits (PR #378). Firm activity/setup and aggregate read budgets implemented in the current change. | Current change's CI/merge; production TLS interoperability, named service identities and gated unattended runs. |
