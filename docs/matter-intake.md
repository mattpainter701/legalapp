...
2. Choose what the client should sign or complete: the fee agreement (optional — a reviewed PDF up to 20 MiB, prepared in Template Studio or elsewhere), the questionnaire questions, and the client intake form. Any subset may be sent. This step does not draft contract language automatically.
3. **Create Matter & Start Intake** saves the matter, creates the secure client portal invitation, creates a native signature request when a fee agreement is included, and queues delivery. The Send step previews the exact branded email and short text the client will receive, rendered from the same code that delivers them; the portal token is only minted on send, so the preview shows a sample link. Email and SMS carry separate copy. If intake preparation fails after matter creation, the modal retries against that saved matter. **Save without sending** remains available.
4. Client opens the emailed/texted invitation, uses the existing portal activation/session flow, reviews and acknowledges the fee agreement when one was sent, and submits the required questionnaire answers. The invitation link is the client portal link; signing unlocks the rest of the portal rather than triggering a second link. Completed questionnaire text is saved as a matter document. The native signature acknowledgment/certificate is the fee agreement evidence; staff can instead verify an uploaded signed agreement received elsewhere. Without a fee agreement the portal is open on the first message.
