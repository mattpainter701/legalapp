...
1. Create a synthetic fixture from the API container:
   ...
   ```bash
   cd /app/backend
   python scripts/seed_demo_fixture.py --domain lawhand-demo-fixture-v1.invalid
   ```

2. Inspect the fixture in the product. Confirm every person, company, case,
...
3. Set these API environment secrets and restart the API processes:
   ...
   DEMO_FIXTURE_TENANT_DOMAIN=lawhand-demo-fixture-v1.invalid
...
- Open **Morgan v. Northstar — Synthetic** and its agreement.
- Ask Standard AI: `What notice period does the agreement require?`
- Confirm the answer uses the private synthetic agreement and the banner count
  advances after refresh.
