...
### Defects the acceptance run found

Running the journey end to end against the real application, rather than fixtures, uncovered six defects that unit tests with mocked transports could not see. A review bot then reported a seventh, a disclosure across portal invites. All are fixed in this PR.

- **Signature completion violated a database constraint.** The completion timeline event was written with no author, while `matter_events.created_by` is `NOT NULL`, so every completed signature failed at commit on PostgreSQL. Completion is now attributed to the staff member who requested the signature, and otherwise to the matter's responsible user; a portal signer is never a firm user.
- **Client paperwork was never sent from the browser.** The shared API client defaults to a JSON content type, so the intake form was posted without a multipart boundary and the server rejected it as missing. The request now sets the multipart header, matching every other upload in the application.
- **An initial paperwork link signed the client out.** The optional mediation add-on is denied to a restricted pre-signing session by design, and the portal treated that denial as an expired session, stranding the client before the fee agreement. Only a genuine `401` now ends the visit.
- **One unreachable calendar stopped every intake follow-up.** The worker expected returned exceptions from calendar cleanup, which instead raises, so a firm with no connected calendar aborted packet processing before its follow-up tasks. Cleanup that cannot be verified now keeps its flag for the next pass and the remaining follow-ups continue.
- **A linked fee agreement inherited the email attachment ceiling.** Selecting an agreement already filed in the matter read it through the 2 MiB mail path even though intake delivers a secure link, while uploading the identical PDF allowed 20 MiB. Reading a reviewed document is now separate from wrapping one as a mail attachment, and intake reads to its own limit. Unreadable documents no longer report themselves as oversized.
- **Selected paperwork was disclosed to every other portal invite.** Starting intake set the matter-wide `portal_visible` bit on the fee agreement and each selected form, so a co-party holding an ordinary invite on the same matter could list and download documents the firm had never shared with them. The packet is now the grant: a recipient reads a document when it is shared with the matter's client or when their own packet entitles them to it, before and after signing. Reported by the review bot on this PR.
- **The default document view had no preview.** Preview existed only in Folder view, so an attorney reviewing a fee agreement in the Detailed list could only download it. Both the Detailed rows and the phone cards now open the same preview.
