...
async def schedule_due_rules(db, tenant_id, *, now=None):
    ...
    for rule in rules:
        ...
        if schedule.kind == "workflow_event":
            source_event = await db.scalar(
                ...
                .where(
                    ...
                    ~MatterWorkflowAutomationEvent.id.in_(
                        select(AutomationServiceOccurrence.source_event_id).where(
                            AutomationServiceOccurrence.tenant_id == tenant_id,
                            AutomationServiceOccurrence.source_event_id.is_not(None),
                        )
                    ),
                )
                .order_by(MatterWorkflowAutomationEvent.created_at)
                .with_for_update(skip_locked=True)
            )
        ...
        existing = await db.scalar(
            select(AutomationServiceOccurrence).where(
                AutomationServiceOccurrence.tenant_id == tenant_id,
                AutomationServiceOccurrence.rule_id == rule.id,
        ...
        identity_count = await db.scalar(
            select(func.count())
            .select_from(AutomationServiceOccurrence)
            .where(
                AutomationServiceOccurrence.tenant_id == tenant_id,
                AutomationServiceOccurrence.identity_id == identity.id,
                AutomationServiceOccurrence.created_at
        ...
        if identity_count and identity_count >= MAX_IDENTITY_DAILY_RUNS:
            row = AutomationServiceOccurrence(
                tenant_id=tenant_id,
                rule_id=rule.id,
                ...
                outcome="blocked",
                failure_code="identity_daily_budget",
            )
            db.add(row)
            outcomes.append(row)
            continue
        ...
        if tenant_count and tenant_count >= MAX_TENANT_DAILY_RUNS:
            row = AutomationServiceOccurrence(
                ...
                rule_sha256=rule.definition_sha256,
                outcome="blocked",
                failure_code="tenant_daily_budget",
            )
            db.add(row)
            outcomes.append(row)
            continue
        service_user = await db.scalar(
            select(User).where(User.tenant_id == tenant_id, User.id == identity.user_id)
        )
        payload = open_payload(
            rule.plan_ciphertext,
            tenant_id=tenant_id,
            run_id=rule.id,
            step_id=rule.id,
            kind="service_plan",
            expected_sha256=rule.payload_sha256,
        )
        plan = WorkflowRunInput.model_validate(payload["plan"]).model_copy(
        ...
        context = CapabilityContext(
            ...
            review_owner_user_id=rule.approved_by_user_id,
            service_rule_id=rule.id,
            service_rule_sha256=rule.definition_sha256,
            allowed_sources=payload.get("sources"),
        )
        try:
