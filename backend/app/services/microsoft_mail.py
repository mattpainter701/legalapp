...
async def _ms_get_user_token(db: AsyncSession, tenant_id: str, user_id: str) -> str | None:
...
async def ms_read_mail_user(
    ...
    async with httpx.AsyncClient() as client:
        ...
        if resp.status_code != 200:
            logger.warning("MS Graph mail read failed: %s %s", resp.status_code, resp.text[:200])
...
async def ms_read_mail_tenant(
    ...
    all_messages = []
    async with httpx.AsyncClient() as client:
        users_resp = await client.get(url, headers={"Authorization": f"Bearer {token}"}, params=params)
        if users_resp.status_code != 200:
            logger.warning("MS Graph user list failed: %s", users_resp.status_code)
            raise RuntimeError(f"Microsoft Graph user list failed: {users_resp.status_code}")
        users = users_resp.json().get("value", [])

        for user in users:
            uid = user["id"]
            try:
                messages = await ms_read_mail_user(db, tenant_id, uid, days, min(max_results, 20))
                for m in messages:
                    m["user_id"] = uid
