    ...
    return digest_payload(
        {
            ...
            "matter_id": str(rule.matter_id),
            "source_run_id": str(rule.source_run_id),
...
async def create_rule(db, *, tenant_id, actor, body: ServiceRuleInput):
    ...
    identity = await db.scalar(
        select(AutomationServiceIdentity)
        .where(
    ...
    run = await db.scalar(
        ...
        .where(
            ...
            WorkflowRun.id == body.source_run_id,
        )
        .with_for_update()
    )
    if not identity or not run:
        raise CapabilityError(
            "service_source_unavailable",
            "Choose a current service identity and completed workflow",
        )
    if identity.status != "active":
        raise CapabilityError(
            "service_identity_disabled",
            "Enable the named service before preparing a rule",
        )
    ...
    if event_rule_id:
        ...
        if not event_rule or event_rule.template_id is None:
            raise CapabilityError(
                "service_event_rule_unavailable", "Choose an active lifecycle rule"
            )
    ...
    rule = AutomationServiceRule(
        ...
        plan_ciphertext=ciphertext,
        payload_sha256=payload_sha256,
        plan_sha256=digest_payload(metadata),
        definition_sha256="0" * 64,
        event_rule_id=event_rule_id,
        event_rule_sha256=event_rule_sha256,
        created_by_user_id=actor.id,
        status="draft",
        version=1,
    )
...
async def approve_rule(db, *, tenant_id, actor, rule_id, expected_version):
    ...
    if "approve_legal_work" not in caps:
        raise CapabilityError(
            "approval_permission_denied", "Legal-work approval is required"
        )
    rule = await db.scalar(
        select(AutomationServiceRule)
        .where(
            AutomationServiceRule.tenant_id == tenant_id,
            AutomationServiceRule.id == rule_id,
        )
        .with_for_update()
    )
    if not rule:
    ...
    if rule.version != expected_version:
        raise CapabilityError(
            ...
            "Reload the service rule before approving it",
        )
    if rule.status != "draft":
        raise CapabilityError(
            "service_rule_not_draft", "Only a reviewed draft can be approved"
        )
    if not actor.is_active or not actor.license_active:
        raise CapabilityError(
            "approval_actor_unavailable", "The approving attorney is unavailable"
        )
    rule.status = "active"
...
async def set_rule_status(db, *, tenant_id, actor, rule_id, expected_version, status):
    if status not in {"active", "paused"}:
        raise CapabilityError(
            "invalid_service_rule_status", "A rule may only be active or paused"
        )
    rule = await db.scalar(
        select(AutomationServiceRule)
        .where(
    ...
    if not rule or rule.version != expected_version:
        raise CapabilityError(
            "service_rule_version_conflict",
            "Reload the service rule before changing it",
        )
    if rule.status == "draft":
        raise CapabilityError(
            "service_rule_not_approved",
            "Approve the rule before changing its schedule state",
        )
