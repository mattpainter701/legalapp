...
async def freeze_service_plan(db, run, granted):
    ...
    if not steps or any(step.status != "completed" for step in steps):
        raise CapabilityError(
            "source_run_incomplete", "Every source step must be completed"
        )
    capabilities = {step.capability for step in steps}
    if not capabilities.issubset(SERVICE_CAPABILITIES & set(granted)):
        raise CapabilityError(
            "service_scope_denied", "The named service cannot repeat every step"
        )
    if not any(resolve_capability_spec(name).mutating for name in capabilities):
        raise CapabilityError(
            "service_plan_has_no_work", "Choose a workflow that prepares review work"
        )
    bindings = {
        (b["kind"], b["id"]): b for b in run.plan_json.get("source_bindings", [])
    }
    prior, frozen = {}, []
    for step in steps:
        spec = resolve_capability_spec(step.capability)
        parsed = spec.parse_arguments(await _arguments(db, run, step, spec)).model_dump(
            mode="json"
        )
        ...
        for argument, kind, model in (
            ...
            if reference:
                ...
                if (
                    ...
                    or earlier
                    not in {"propose_matter_document", "propose_document_from_template"}
                    or reference["path"] != ["document_id"]
                ):
                    raise CapabilityError(
                        "mutable_service_source",
                        "Select an explicit document or template before approving a repeating rule",
                    )
            elif parsed.get(argument):
                identifier = uuid.UUID(parsed[argument])
                query = select(model).where(
                    model.tenant_id == run.tenant_id, model.id == identifier
                )
                if model is MatterDocument:
                    query = query.where(
                        model.matter_id == run.matter_id,
                        model.storage_state == "verified",
                    )
                source = await db.scalar(query)
                digest = getattr(
                    source,
                    "source_sha256" if model is DocumentTemplate else "document_sha256",
                    None,
                )
                if not digest:
                    raise CapabilityError(
                        "unverified_service_source",
                        "The repeatable source needs verified content",
                    )
                key = (kind, str(identifier))
                if key in bindings and bindings[key]["sha256"] != digest:
                    raise CapabilityError(
                        "source_binding_changed",
                        "The completed workflow's source changed",
                    )
        ...
        frozen.append(
            {
                ...
                "arguments": parsed,
                "references": step.references_json,
            }
        )
