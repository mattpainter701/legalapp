...
_FORBIDDEN_DURABLE_KEYS = {
    "body", "content", "document_text", "raw_text", "source_text", "value", "values",
    "default", "example", "provider_path", "provider_id", "storage_path",
    "signed_url", "download_url", "source_url", "item_id", "drive_id",
}
_RESERVED_FIELD_DEFINITION_KEYS = {
    "id", "studio_field_id", "name", "key", "automation_key", "label",
    "type", "field_type", "required", "position", "placements",
}
_ALLOWED_FIELD_DEFINITION_KEYS = {
    "choices", "confidence", "constraints", "date_format", "display",
    "included", "max_length", "maximum", "min_length", "minimum",
    "multiline", "number_format", "option_labels", "pattern", "readonly",
...
def _canonical_json(value: Any) -> bytes:
    return json.dumps(
        jsonable_encoder(value), sort_keys=True, separators=(",", ":"), ensure_ascii=False
    ).encode("utf-8")
...
def _bounded_redacted(value: Any) -> Any:
    ...
    def walk(item: Any, depth: int) -> Any:
        ...
        if depth > MAX_JSON_DEPTH or nodes > MAX_JSON_NODES:
            raise StudioError(422, {"code": "payload_too_complex", "message": "JSON payload exceeds structural limits"})
        if isinstance(item, dict):
            clean: dict[str, Any] = {}
            for raw_key, child in item.items():
                key = str(raw_key)
                if key.lower() in _FORBIDDEN_DURABLE_KEYS:
                    raise StudioError(422, {"code": "unsafe_durable_payload", "message": f"durable key '{key}' is not allowed"})
                clean[key] = walk(child, depth + 1)
            return clean
        if isinstance(item, list):
            if len(item) > 1000:
                raise StudioError(422, {"code": "payload_too_large", "message": "array exceeds item limit"})
            return [walk(child, depth + 1) for child in item]
        if isinstance(item, str) and len(item) > 2000:
            raise StudioError(422, {"code": "payload_too_large", "message": "string exceeds durable limit"})
        if item is None or isinstance(item, (str, int, float, bool)):
            return item
        raise StudioError(422, {"code": "unsupported_payload_type", "message": "payload must be JSON-compatible"})

    result = walk(jsonable_encoder(value), 0)
    if len(_canonical_json(result)) > MAX_JSON_BYTES:
        raise StudioError(422, {"code": "payload_too_large", "message": "JSON payload exceeds byte limit"})
    return result
...
class StudioDraftService:
    ...
    def __init__(self, db: AsyncSession, tenant_id: uuid.UUID, actor_user_id: uuid.UUID):
        self.db = db
    ...
    async def _idempotency(
        ...
        if row:
            if row.request_sha256 != request_hash:
                raise StudioError(409, {
                    "code": "idempotency_key_mismatch",
                    "message": "idempotency key was already used for a different request",
                })
            if row.response_json is not None:
                return None, row.response_json
            raise StudioError(409, {"code": "request_in_progress", "message": "matching request is still in progress"})
        reservation = StudioDraftIdempotency(
            ...
            expires_at=datetime.now(timezone.utc) + timedelta(
                hours=self.settings.TEMPLATE_STUDIO_IDEMPOTENCY_TTL_HOURS
            ),
        )
    ...
    async def _ensure_source_artifact(
        ...
        artifact = await self.db.scalar(select(StudioSourceArtifact).where(
            StudioSourceArtifact.id == artifact_id,
            StudioSourceArtifact.tenant_id == self.tenant_id,
        ))
        if artifact is None:
            self.db.add(StudioSourceArtifact(
                id=artifact_id, tenant_id=self.tenant_id, sha256=sha256,
                media_type=media_type, created_by_user_id=self.actor_user_id,
            ))
            await self.db.flush()
            return
    ...
    async def _parts(
        ...
        fields = list((await self.db.scalars(
        ...
        placements = list((await self.db.scalars(
    ...
    def _identity_payload(
        ...
        return {
            ...
            "format": draft.format,
            "fields": [{
                "id": str(item.id), "automation_key": item.automation_key,
                "label": item.label, "field_type": item.field_type,
                "required": item.required, "position": item.position,
                "definition": item.definition,
            } for item in fields],
            "placements": [{
                "id": str(item.id), "field_id": str(item.field_id),
                "format": item.format, "anchor_kind": item.anchor_kind,
                "anchor": item.anchor,
    ...
    async def _response(self, draft: StudioDraft) -> dict[str, Any]:
        ...
        return {
            ...
            "fields": [{
                "id": item.id, "automation_key": item.automation_key,
                "label": item.label, "field_type": item.field_type,
                "required": item.required, "position": item.position,
                "definition": item.definition,
            } for item in fields],
            "placements": [{
                "id": item.id, "field_id": item.field_id, "format": item.format,
                "anchor_kind": item.anchor_kind, "anchor": item.anchor,
            } for item in placements],
    ...
    def _audit(
        self, draft: StudioDraft, event: str, base_revision: int | None, detail: dict[str, Any]
    ) -> None:
        self.db.add(StudioDraftAuditEvent(
            tenant_id=self.tenant_id, draft_id=draft.id, event_type=event,
            revision=draft.revision, base_revision=base_revision,
            actor_user_id=self.actor_user_id, detail=_bounded_redacted(detail),
        ))

    async def read(self, draft_id: uuid.UUID) -> dict[str, Any]:
        draft = await self.db.scalar(select(StudioDraft).where(
            StudioDraft.id == draft_id, StudioDraft.tenant_id == self.tenant_id
        ))
        if draft is None:
            raise StudioError(404, "Template Studio draft not found")
    ...
    async def create(
        self, request: StudioDraftCreate, idempotency_key: str,
        *, operation: str = "create",
    ) -> dict[str, Any]:
        reservation, replay = await self._idempotency(operation, idempotency_key, request)
        if replay is not None:
            return replay
        count = await self.db.scalar(select(func.count()).select_from(StudioDraft).where(
        ...
        if (count or 0) >= self.settings.TEMPLATE_STUDIO_ACTIVE_DRAFT_QUOTA:
            raise StudioError(429, {"code": "draft_quota_exceeded", "message": "tenant active draft quota reached"})

        if request.published_template_id is not None:
            template = await self.db.scalar(select(DocumentTemplate).where(
            ...
            template_hash = template.source_sha256 or hashlib.sha256(template.body.encode("utf-8")).hexdigest()
            if template_hash != request.source_sha256:
                raise StudioError(409, {"code": "source_hash_mismatch", "message": "draft source does not match the published compatibility record"})
        ...
        draft = StudioDraft(
            ...
            title=request.title.strip(), format=request.format,
        ...
        for field in request.fields:
            self.db.add(StudioDraftField(
                id=field.id or uuid.uuid4(), tenant_id=self.tenant_id, draft_id=draft.id,
                automation_key=field.automation_key, label=field.label,
                field_type=field.field_type, required=field.required,
                position=field.position, definition=_field_definition(field.definition),
            ))
        for placement in request.placements:
            self.db.add(StudioDraftPlacement(
                id=placement.id or uuid.uuid4(), tenant_id=self.tenant_id,
                draft_id=draft.id, field_id=placement.field_id, format=placement.format,
                anchor_kind=placement.anchor_kind, anchor=_bounded_redacted(placement.anchor),
            ))
        await self.db.flush()
        await self._refresh_identity(draft)
        self._audit(draft, "created", None, {
            "field_count": len(request.fields), "placement_count": len(request.placements),
    ...
    async def import_template(
        ...
        if template is None:
            raise StudioError(404, "Published template not found")
        source_sha = template.source_sha256 or hashlib.sha256(template.body.encode("utf-8")).hexdigest()
        raw_fields = (template.variable_schema or {}).get("fields") or []
        if not isinstance(raw_fields, list) or len(raw_fields) > 200:
            raise StudioError(422, {"code": "invalid_variable_schema", "message": "published variable schema is not safely importable"})
        fields = []
        placements: list[dict[str, Any]] = []
        for position, raw in enumerate(raw_fields):
            if not isinstance(raw, dict):
                raise StudioError(422, {"code": "invalid_variable_schema", "message": "field entries must be objects"})
            key = str(raw.get("name") or raw.get("key") or "").strip()
            if not key:
                raise StudioError(422, {"code": "invalid_variable_schema", "message": "field key is required"})
            try:
                field_id = uuid.UUID(str(raw["studio_field_id"])) if raw.get("studio_field_id") else uuid.uuid4()
            ...
            fields.append({
                "id": field_id, "automation_key": key,
            ...
            if raw.get("pdf_field_name"):
                placements.append({
                    "id": uuid.uuid4(), "field_id": field_id, "format": "pdf",
            ...
            for overlay in overlay_items[:20]:
                if isinstance(overlay, dict):
                    ...
                    placements.append({
                        "id": uuid.uuid4(), "field_id": field_id, "format": "pdf",
                        "anchor_kind": "overlay", "anchor": _bounded_redacted(clean_overlay),
                    })
            if isinstance(raw.get("docx_anchor"), dict):
                clean_anchor = {
                    k: v for k, v in raw["docx_anchor"].items()
                    if k not in {"source_text", "text", "value", "default"}
                }
                placements.append({
                    "id": uuid.uuid4(), "field_id": field_id, "format": "docx",
                    "anchor_kind": "docx_anchor", "anchor": _bounded_redacted(clean_anchor),
                })
                ...
                placements.append({
                    ...
                    "anchor_kind": "template_token", "anchor": {"token": key},
                })
        try:
            create_request = StudioDraftCreate.model_validate({
                ...
                "source_media_type": template.source_content_type or "text/markdown",
        ...
        return await self.create(create_request, idempotency_key, operation="import_template")

    async def patch(
        self, draft_id: uuid.UUID, request: StudioDraftPatch, idempotency_key: str,
        *, operation: str = "patch",
    ) -> dict[str, Any]:
        ...
        for item in request.operations:
            ...
            if item.op == "set_metadata":
                ...
                if payload.id is not None:
                    scoped_row = await self.db.scalar(select(StudioDraftField).where(
                    ...
                    row = scoped_row
                duplicate_key = await self.db.scalar(select(StudioDraftField.id).where(
                ...
                if row is None:
                    row = StudioDraftField(
                        id=payload.id or uuid.uuid4(), tenant_id=self.tenant_id,
                        draft_id=draft.id, automation_key=payload.automation_key,
                        label=payload.label, field_type=payload.field_type,
                ...
                result = await self.db.execute(delete(StudioDraftField).where(
                ...
                field_exists = await self.db.scalar(select(StudioDraftField.id).where(
                    StudioDraftField.id == payload.field_id,
                    StudioDraftField.draft_id == draft.id,
                    StudioDraftField.tenant_id == self.tenant_id,
                ))
                if field_exists is None:
                    raise StudioError(422, {"code": "unknown_field", "message": "placement field is not in this draft"})
                row = None
                if payload.id is not None:
                    scoped_row = await self.db.scalar(select(StudioDraftPlacement).where(
                ...
                if row is None:
                    row = StudioDraftPlacement(
                        id=payload.id or uuid.uuid4(), tenant_id=self.tenant_id,
                        draft_id=draft.id, field_id=payload.field_id,
                        format=payload.format, anchor_kind=payload.anchor_kind,
                ...
                result = await self.db.execute(delete(StudioDraftPlacement).where(
        ...
        self._audit(draft, "patched", request.base_revision, {
    ...
    async def validate(
        ...
        for field in fields:
            if field.automation_key in keys:
                issues.append({"code": "duplicate_automation_key", "field_id": str(field.id)})
            keys.add(field.automation_key)
        field_ids = {field.id for field in fields}
        for placement in placements:
            if placement.field_id not in field_ids:
                issues.append({"code": "orphan_placement", "placement_id": str(placement.id)})
            if placement.format != draft.format:
                issues.append({"code": "placement_format_mismatch", "placement_id": str(placement.id)})
    ...
    async def snapshot(
        ...
        draft = await self._locked_draft(draft_id)
        self._check_revision(draft, request.expected_revision)
        existing = await self.db.scalar(select(StudioDraftSnapshot).where(
        ...
        if existing is not None:
            response = {
                "id": existing.id, "draft_id": draft.id, "revision": existing.revision,
                "identity_sha256": existing.identity_sha256,
                "content_sha256": existing.content_sha256, "payload": existing.payload,
            }
            self._finish_idempotency(reservation, response)
            await self.db.commit()
            return response
        snapshot_count = await self.db.scalar(select(func.count()).select_from(StudioDraftSnapshot).where(
            StudioDraftSnapshot.tenant_id == self.tenant_id,
            StudioDraftSnapshot.draft_id == draft.id,
        ))
        if (snapshot_count or 0) >= self.settings.TEMPLATE_STUDIO_SNAPSHOT_QUOTA:
            raise StudioError(429, {"code": "snapshot_quota_exceeded", "message": "draft snapshot quota reached"})
        fields, placements = await self._parts(draft.id)
        payload = _bounded_redacted({
            "contract_version": 1, "draft_id": str(draft.id),
            "revision": draft.revision, "identity_sha256": draft.identity_sha256,
            "title": draft.title, "format": draft.format,
            "lifecycle_state": draft.lifecycle_state,
            "source": source_contract(draft),
            "fields": self._identity_payload(draft, fields, placements)["fields"],
            "placements": self._identity_payload(draft, fields, placements)["placements"],
        })
        content_hash = _sha256(payload)
        snapshot = StudioDraftSnapshot(
            tenant_id=self.tenant_id, draft_id=draft.id, revision=draft.revision,
            identity_sha256=draft.identity_sha256, content_sha256=content_hash,
            payload=payload, created_by_user_id=self.actor_user_id,
        )
        self.db.add(snapshot)
        await self.db.flush()
        self._audit(draft, "snapshot_created", draft.revision, {
            "snapshot_id": str(snapshot.id), "content_sha256": content_hash,
        })
        await self.db.flush()
        response = {
            "id": snapshot.id, "draft_id": draft.id, "revision": snapshot.revision,
            "identity_sha256": snapshot.identity_sha256,
            "content_sha256": snapshot.content_sha256, "payload": snapshot.payload,
        }
        self._finish_idempotency(reservation, response)
        await self.db.commit()
        return response

    async def read_snapshot(self, draft_id: uuid.UUID, snapshot_id: uuid.UUID) -> dict[str, Any]:
        row = await self.db.scalar(select(StudioDraftSnapshot).where(
        ...
        return {
            "id": row.id, "draft_id": row.draft_id, "revision": row.revision,
            "identity_sha256": row.identity_sha256,
            "content_sha256": row.content_sha256, "payload": row.payload,
        }
    ...
    @staticmethod
    def variable_schema(fields: list[StudioDraftField], placements: list[StudioDraftPlacement]) -> dict[str, Any]:
        by_field: dict[uuid.UUID, list[dict[str, Any]]] = {}
        for item in placements:
            by_field.setdefault(item.field_id, []).append({
                "studio_placement_id": str(item.id), "format": item.format,
                "anchor_kind": item.anchor_kind, "anchor": item.anchor,
            })
        return {"version": 2, "fields": [{
            "studio_field_id": str(field.id), "name": field.automation_key,
    ...
    async def promote(
        ...
        draft = await self._locked_draft(draft_id)
        self._check_revision(draft, request.expected_revision)
        if draft.lifecycle_state != "active" or draft.cancellation_requested_at is not None:
            raise StudioError(409, {"code": "draft_not_publishable", "message": "archived or cancelled drafts cannot be promoted"})
        if draft.published_template_id is None:
            raise StudioError(422, {"code": "materialization_required", "message": "new-template materialization is owned by a later publish phase"})
        template = await self.db.scalar(select(DocumentTemplate).where(
        ...
        if template is None:
            raise StudioError(404, "Published template not found")
        current_source_hash = template.source_sha256 or hashlib.sha256(template.body.encode("utf-8")).hexdigest()
        if current_source_hash != draft.source_sha256:
            raise StudioError(409, {"code": "source_hash_mismatch", "message": "published source changed outside this draft"})
        fields, placements = await self._parts(draft.id)
        template.title = draft.title
        template.variable_schema = _bounded_redacted(self.variable_schema(fields, placements))
        ...
        self._audit(draft, "promoted", base, {
            "template_id": str(template.id), "published_status": request.status,
        })
        await self.db.flush()
        response = await self._response(draft)
        self._finish_idempotency(reservation, response)
    ...
    async def purge_expired_idempotency(self) -> int:
        result = await self.db.execute(delete(StudioDraftIdempotency).where(
            StudioDraftIdempotency.tenant_id == self.tenant_id,
