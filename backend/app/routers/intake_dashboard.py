...
async def search_dashboard(
    ...
    if matter_matchers:
        ...
        for matter in matters:
            attorney_name = await _user_name(
                db, matter.partner_attorney_id or matter.attorney_of_record_id
            )
            results.append(
                IntakeSearchResult(
                    id=str(matter.id),
    ...
    return IntakeDashboardSearchResponse(
        ...
        recommended_attorney_name=recommended,
        results=ordered,
    )
...
async def assign_next_partner(
    ...
    if not rule:
        raise HTTPException(status_code=404, detail="No rotation rule for practice area")
