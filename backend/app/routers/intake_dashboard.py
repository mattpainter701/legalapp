...
async def _upsert_general_call_task(
    ...
    *,
    ...
    description_bits = [
        ...
        f"Phone: {_log_participant(log, 'phone')}" if _log_participant(log, "phone") else "",
...
async def export_call_records(
    ...
    for log, contact, creator in log_rows:
        ...
        task = task_by_log_id.get(log.id) or (task_by_lead_id.get(lead.id) if lead else None)
        assigned_to_user_id = None
        if task and task.assigned_to_user_id:
            assigned_to_user_id = task.assigned_to_user_id
        elif lead and lead.assigned_to_user_id:
...
async def create_dashboard_call(
    ...
    if payload.task_mode == "partner_rotation" and payload.assigned_to_user_id and not lead_assignee:
    ...
    if payload.task_mode == "specific_staff":
        ...
        if not general_task_assignee:
            raise HTTPException(status_code=422, detail="Select an active staff member for the task")
