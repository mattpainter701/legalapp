...
async def stream_message(
    ...
    """
    Streaming RAG endpoint — processes a user message and streams LLM response tokens via SSE.
    Returns Server-Sent Events with each token as it arrives from the LLM.
    """
    user = await get_current_user(request, db)
    await set_tenant_context(db, str(user.tenant_id))
    ...
    async def stream_generator():
        """Generator that yields SSE-formatted tokens from the LLM stream."""
        accumulated_text = ""
        try:
            async for token in llm_service.stream_complete(
                messages=history_messages,
        ...
        # After stream completes, save message and perform guardrails check
        try:
            # Apply guardrails to the full accumulated response
            cleaned_response, needs_retry, response_pii = apply_guardrails(
                accumulated_text, privacy_mode=user.privacy_mode
            )
            ...
            # Record usage (for streaming, we estimate tokens)
            tokens_in, tokens_out = (
                0,
                0,
            )  # Streaming doesn't provide token counts in real-time
            cost = calculate_cost(
                tokens_in=tokens_in,
                tokens_out=tokens_out,
                model=model_used,
            ...
            usage = UsageRecord(
                ...
                user_agent=(request.headers.get("user-agent") or "")[:500] or None,
                cache_hit_rag=cache_hit_rag,
                cache_hit_llm=False,  # No LLM cache for streaming
                cache_hit_matter=cache_hit_matter,
            )
            db.add(usage)
            ...
            logging.error(f"Error saving streamed message: {e}")
