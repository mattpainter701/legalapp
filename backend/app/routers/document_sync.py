...
async def sync_and_ingest(
    ...
    payload["user_id"] = payload.get("user_id") or str(admin_user.id)
    requested_key = request.headers.get("Idempotency-Key")
    key = requested_key or str(uuid.uuid4())
    job = await enqueue_job(
        db,
        tenant_id=tenant_id,
        kind="cloud_sync_ingest",
        idempotency_key=key,
        payload=payload,
    )
    await db.commit()
    ...
    return {
        "status": job.status,
        "job_id": str(job.id),
        "message": "Document sync and ingestion queued durably",
    }
...
async def get_sync_job(
    ...
    if not tenant_id:
        raise HTTPException(status_code=400, detail="No tenant context")
    await set_tenant_context(db, tenant_id)
    job = await get_tenant_job(db, tenant_id, job_id)
    if not job:
