...
async def list_documents(
    ...
    if body.provider == "onedrive":
        docs = await document_sync.sync_onedrive(
            db, tenant_id, str(user.id) if not body.user_id else body.user_id, max_files=body.max_files
        )
    ...
    if body.download_and_index:
        for doc in docs:
            try:
                path = await document_sync.download_and_process(db, tenant_id, doc, str(user.id))
                if path:
                    downloaded += 1
...
async def sync_and_ingest(
    ...
    async def _ingest():
        ...
        async with async_session_maker() as session:
            try:
                ...
                if body.provider == "onedrive":
                    docs = await document_sync.sync_onedrive(session, tenant_id, max_files=body.max_files)
                elif body.provider == "sharepoint":
                    docs = await document_sync.sync_sharepoint(session, tenant_id, site_id=body.site_id, max_files=body.max_files)
                elif body.provider == "google_drive":
                    docs = await document_sync.sync_google_drive(session, tenant_id, body.user_id, max_files=body.max_files)
                else:
                    return

                for doc in docs:
                    try:
                        local_path = await document_sync.download_and_process(session, tenant_id, doc)
                        if local_path:
    ...
    return {"status": "started", "message": "Document sync and ingestion running in background"}
