...
async def list_services(db=Depends(get_db), user=Depends(manage)):
    ...
    identities = (
        await db.scalars(
            ...
            .where(AutomationServiceIdentity.tenant_id == user.tenant_id)
            .order_by(AutomationServiceIdentity.created_at.desc())
        )
    ).all()
    rules = (
        await db.scalars(
            select(AutomationServiceRule)
            .where(AutomationServiceRule.tenant_id == user.tenant_id)
            .order_by(AutomationServiceRule.created_at.desc())
        )
    ).all()
    return {
        "identities": [identity_response(row) for row in identities],
...
async def add_identity(
    body: ServiceIdentityInput, db=Depends(get_db), user=Depends(manage)
):
    await set_tenant_context(db, str(user.tenant_id))
    try:
...
async def approve(
    rule_id: uuid.UUID, body: VersionRequest, db=Depends(get_db), user=Depends(review)
):
    await set_tenant_context(db, str(user.tenant_id))
    try:
        row = await approve_rule(
            db,
...
async def status(
    rule_id: uuid.UUID, body: StatusRequest, db=Depends(get_db), user=Depends(manage)
):
    await set_tenant_context(db, str(user.tenant_id))
    try:
        row = await set_rule_status(
            db,
            tenant_id=user.tenant_id,
            actor=user,
            rule_id=rule_id,
