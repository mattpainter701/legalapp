...
class AutomationServiceIdentity(RunIdentity, Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "id", name="uq_automation_service_identity_tenant"),
        UniqueConstraint("tenant_id", "name", name="uq_automation_service_identity_name"),
        UniqueConstraint("user_id", name="uq_automation_service_identity_user"),
        tenant_fk("user_id", "users"), tenant_fk("created_by_user_id", "users"),
        CheckConstraint("status IN ('active','disabled')", name="ck_automation_service_identity_status"),
        CheckConstraint("jsonb_typeof(capabilities)='array' AND jsonb_array_length(capabilities) BETWEEN 1 AND 10", name="ck_automation_service_identity_grant"),
...
class AutomationServiceRule(RunIdentity, Base):
    ...
    __table_args__ = (
        ...
        tenant_fk("identity_id", "automation_service_identities"), tenant_fk("matter_id", "matters"),
        tenant_fk("source_run_id", "workflow_runs"), tenant_fk("event_rule_id", "matter_workflow_automation_rules"),
        tenant_fk("created_by_user_id", "users"), tenant_fk("approved_by_user_id", "users"),
        CheckConstraint("status IN ('draft','active','paused')", name="ck_automation_service_rule_status"),
        CheckConstraint("(status='draft')=(approved_by_user_id IS NULL AND approved_at IS NULL)", name="ck_automation_service_rule_approval"),
        CheckConstraint("(approved_by_user_id IS NULL)=(approved_at IS NULL)", name="ck_automation_service_rule_approval_pair"),
        CheckConstraint("definition_sha256 ~ '^[a-f0-9]{64}$' AND plan_sha256 ~ '^[a-f0-9]{64}$' AND payload_sha256 ~ '^[a-f0-9]{64}$'", name="ck_automation_service_rule_hashes"),
        CheckConstraint("(event_rule_id IS NULL)=(event_rule_sha256 IS NULL)", name="ck_automation_service_rule_event_pair"),
...
class AutomationServiceOccurrence(RunIdentity, Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "rule_id", "occurrence_key", name="uq_automation_service_occurrence"),
        UniqueConstraint("run_id", name="uq_automation_service_occurrence_run"),
        tenant_fk("rule_id", "automation_service_rules"), tenant_fk("identity_id", "automation_service_identities"),
        tenant_fk("run_id", "workflow_runs"),
        CheckConstraint("outcome IN ('started','blocked','skipped')", name="ck_automation_service_occurrence_outcome"),
        CheckConstraint("(outcome='started')=(run_id IS NOT NULL)", name="ck_automation_service_occurrence_run"),
        CheckConstraint("rule_sha256 ~ '^[a-f0-9]{64}$'", name="ck_automation_service_occurrence_hash"),
        Index("ix_automation_service_occurrences_budget", "tenant_id", "identity_id", "created_at"),
    )
