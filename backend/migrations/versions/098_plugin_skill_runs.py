...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("matter_id", UUID(as_uuid=True), sa.ForeignKey("matters.id", ondelete="SET NULL")),
        ...
        sa.Column("gates_triggered", sa.JSON(), nullable=False, server_default=sa.text("'[]'::json")),
        ...
        sa.Column("requires_attorney_review", sa.Boolean(), nullable=False, server_default=sa.text("true")),
        ...
        sa.CheckConstraint("review_status IN ('draft', 'approved', 'rejected')", name="ck_plugin_skill_runs_review_status"),
    )
    op.create_index("idx_plugin_skill_runs_tenant_created", "plugin_skill_runs", ["tenant_id", "created_at"])
    op.create_index("idx_plugin_skill_runs_matter_created", "plugin_skill_runs", ["matter_id", "created_at"])
...
def downgrade() -> None:
    op.drop_index("idx_plugin_skill_runs_matter_created", table_name="plugin_skill_runs")
    op.drop_index("idx_plugin_skill_runs_tenant_created", table_name="plugin_skill_runs")
