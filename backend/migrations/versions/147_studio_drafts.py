...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("tenant_id", "id", "sha256", "media_type", name="uq_studio_source_artifacts_contract"),
        sa.CheckConstraint("sha256 ~ '^[0-9a-f]{64}$'", name="ck_studio_source_artifacts_hash"),
    )
    op.create_index("ix_studio_source_artifacts_tenant_created", "studio_source_artifacts", ["tenant_id", "created_at"])
    ...
    op.create_table(
        ...
        sa.Column("lifecycle_state", sa.String(20), server_default="active", nullable=False),
        ...
        sa.ForeignKeyConstraint(["published_template_id"], ["document_templates.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(["created_by_user_id"], ["users.id"], ondelete="SET NULL"),
        ...
        sa.CheckConstraint("lifecycle_state IN ('active', 'archived')", name="ck_studio_drafts_lifecycle"),
        sa.CheckConstraint("source_sha256 ~ '^[0-9a-f]{64}$' AND identity_sha256 ~ '^[0-9a-f]{64}$'", name="ck_studio_drafts_hashes"),
    )
    op.create_index("ix_studio_drafts_tenant_updated", "studio_drafts", ["tenant_id", "updated_at"])
    ...
    op.create_table(
        ...
        sa.Column("definition", sa.JSON(), server_default=sa.text("'{}'::json"), nullable=False),
        sa.ForeignKeyConstraint(["tenant_id", "draft_id"], ["studio_drafts.tenant_id", "studio_drafts.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("tenant_id", "draft_id", "id", name="uq_studio_fields_scope"),
    ...
    op.create_index("ix_studio_fields_draft_position", "studio_draft_fields", ["tenant_id", "draft_id", "position"])
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("tenant_id", "draft_id", "id", name="uq_studio_placements_scope"),
    )
    op.create_index("ix_studio_placements_field", "studio_draft_placements", ["tenant_id", "draft_id", "field_id"])
    ...
    op.create_table(
        ...
        sa.Column("created_by_user_id", u),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()"), nullable=False),
        sa.ForeignKeyConstraint(["tenant_id", "draft_id"], ["studio_drafts.tenant_id", "studio_drafts.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["created_by_user_id"], ["users.id"], ondelete="SET NULL"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("draft_id", "revision", name="uq_studio_snapshots_revision"),
        sa.UniqueConstraint("draft_id", "content_sha256", name="uq_studio_snapshots_content"),
        sa.CheckConstraint("revision > 0", name="ck_studio_snapshots_revision"),
        sa.CheckConstraint("content_sha256 ~ '^[0-9a-f]{64}$'", name="ck_studio_snapshots_hash"),
    )
    op.create_index("ix_studio_snapshots_draft_created", "studio_draft_snapshots", ["tenant_id", "draft_id", "created_at"])
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()"), nullable=False),
        sa.ForeignKeyConstraint(["tenant_id", "actor_user_id"], ["users.tenant_id", "users.id"], ondelete="RESTRICT"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("tenant_id", "actor_user_id", "operation", "idempotency_key", name="uq_studio_idempotency_key"),
        sa.CheckConstraint("request_sha256 ~ '^[0-9a-f]{64}$'", name="ck_studio_idempotency_hash"),
    )
    op.create_index("ix_studio_idempotency_expires", "studio_draft_idempotency", ["tenant_id", "expires_at"])
    ...
    op.create_index("ix_studio_audit_draft_created", "studio_draft_audit_events", ["tenant_id", "draft_id", "created_at"])
    ...
    op.execute("""
        ...
        $$ LANGUAGE plpgsql
    """)
    for table in ("studio_source_artifacts", "studio_drafts", "studio_draft_snapshots", "studio_draft_audit_events"):
        op.execute(f"CREATE TRIGGER {table}_user_tenant_trigger BEFORE INSERT OR UPDATE ON {table} FOR EACH ROW EXECUTE FUNCTION validate_studio_optional_users()")
    ...
    op.execute("""
        ...
        $$ LANGUAGE plpgsql
    """)
    op.execute("CREATE TRIGGER studio_drafts_source_identity_guard BEFORE UPDATE OF source_artifact_id, source_sha256, source_media_type ON studio_drafts FOR EACH ROW EXECUTE FUNCTION guard_studio_source_identity()")
    ...
    op.execute("""
        ...
        $$ LANGUAGE plpgsql
    """)
    for table in ("studio_source_artifacts", "studio_draft_snapshots", "studio_draft_audit_events"):
        op.execute(f"CREATE TRIGGER {table}_immutable BEFORE UPDATE OR DELETE ON {table} FOR EACH ROW EXECUTE FUNCTION prevent_studio_immutable_mutation()")


def downgrade() -> None:
    for table in ("studio_draft_audit_events", "studio_draft_snapshots", "studio_source_artifacts"):
        op.execute(f"DROP TRIGGER IF EXISTS {table}_immutable ON {table}")
    op.execute("DROP TRIGGER IF EXISTS studio_drafts_source_identity_guard ON studio_drafts")
    for table in ("studio_draft_audit_events", "studio_draft_snapshots", "studio_drafts", "studio_source_artifacts"):
