...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_mcp_product_keys_key_hash", "mcp_product_keys", ["key_hash"], unique=True
    )
    op.create_index(
        "ix_mcp_product_keys_key_prefix", "mcp_product_keys", ["key_prefix"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "product_key_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("mcp_product_keys.id", ondelete="SET NULL"),
            nullable=True,
        ),
        sa.Column(
            "user_id",
        ...
        sa.Column(
            "transport", sa.String(length=40), nullable=False, server_default="rest"
        ),
    ...
    op.create_index(
        "ix_mcp_usage_events_product_key_id", "mcp_usage_events", ["product_key_id"]
    )
    op.create_index(
        "ix_mcp_usage_events_created_at", "mcp_usage_events", ["created_at"]
    )
    op.create_index(
