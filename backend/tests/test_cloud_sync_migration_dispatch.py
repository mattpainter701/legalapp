...
@pytest.mark.asyncio
@pytest.mark.parametrize('provider,group', [('google_drive', 'google'), ('onedrive', 'microsoft'), ('sharepoint', 'microsoft')])
async def test_migrated_sync_only_inventories_bound_target_root(monkeypatch, provider, group):
    service = CloudSyncService()
    monkeypatch.setattr('app.services.cloud_sync.set_tenant_context', AsyncMock())
    service._latest_completed_migration = AsyncMock(return_value=SimpleNamespace(target_provider=provider))
    reindex = AsyncMock(return_value={'status': 'completed', 'items': 7})
    monkeypatch.setattr(storage_migration_reindex, 'run', reindex)
    service.sync_google_drive = AsyncMock(side_effect=AssertionError('must use bounded inventory'))
    service.sync_onedrive = AsyncMock(side_effect=AssertionError('must use bounded inventory'))
    service.sync_sharepoint = AsyncMock(side_effect=AssertionError('must use bounded inventory'))
...
async def test_failed_migrated_sync_reports_failure_to_scheduler(monkeypatch):
    monkeypatch.setattr('app.services.cloud_sync.set_tenant_context', AsyncMock())
    service = CloudSyncService()
    service._latest_completed_migration = AsyncMock(return_value=SimpleNamespace(target_provider='google_drive'))
    monkeypatch.setattr(storage_migration_reindex, 'run', AsyncMock(return_value={'status': 'failed', 'items': 0}))
...
async def test_retired_email_cannot_repopulate_after_cutover():
    ...
    await service._upsert(db, str(uuid4()), provider='microsoft', object_type='email', object_id='old-email')
