...
async def test_studio_drafts_force_rls_isolates_reads_and_writes(test_engine):
    ...
    async with test_engine.begin() as conn:
        await conn.execute(
            text("DROP POLICY IF EXISTS studio_test_tenant_policy ON studio_drafts")
        )
        await conn.execute(text("ALTER TABLE studio_drafts DISABLE ROW LEVEL SECURITY"))
        await conn.execute(
            text(
                "INSERT INTO tenants (id, name, domain, billing_tier, is_active) VALUES (:id, 'Studio A', :domain, 'payg', true)"
            ),
            {"id": tenant_a, "domain": f"studio-a-{tenant_a}.invalid"},
        )
        await conn.execute(
            text(
                "INSERT INTO tenants (id, name, domain, billing_tier, is_active) VALUES (:id, 'Studio B', :domain, 'payg', true)"
            ),
            {"id": tenant_b, "domain": f"studio-b-{tenant_b}.invalid"},
        )
    ...
    try:
        ...
        async with factory() as session:
            ...
            visible = (
                (
                    await session.execute(
                        text("SELECT id FROM studio_drafts ORDER BY id")
                    )
                )
                .scalars()
                .all()
        ...
        async with test_engine.begin() as conn:
            ...
            await conn.execute(
                text("ALTER TABLE studio_drafts DISABLE ROW LEVEL SECURITY")
            )
            await conn.execute(
                text("DELETE FROM studio_drafts WHERE tenant_id IN (:a, :b)"),
                {"a": tenant_a, "b": tenant_b},
            )
            await conn.execute(
                text("DELETE FROM studio_source_artifacts WHERE tenant_id IN (:a, :b)"),
                {"a": tenant_a, "b": tenant_b},
            )
            await conn.execute(
