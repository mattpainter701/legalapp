...
while the tenant has an active `mediation-legal` entitlement.
...
- frontend `npm run check`: 445 tests, lint with two pre-existing `no-alert`
  warnings and no errors, production build passed
- focused backend static/unit contracts: 32 passed
- mediation and native-portal PostgreSQL integration suite: 16 tests collect;
