...
- Document the split in `docs/mcp_security_operations.md`: the epoch gates the workspace and research MCP *consent* routes, which authenticate with the firm session, but an MCP access token already issued to a connected assistant authenticates with its own audience-bound credential and is untouched. A password reset therefore does not disconnect Claude, ChatGPT or Codex — the grant has to be revoked explicitly, which the incident-response steps now say. `docs/session_lifetime.md` cross-references it.
