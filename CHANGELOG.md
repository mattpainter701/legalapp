...
- Add `frontend/src/pages/ClientPortalLoginPage.jsx` and the `/portal/client/login` route (with `/portal/client` redirecting to it), and offer account activation on the accept page. `activateClientPortalAccount` and `loginClientPortalAccount` were already exported from `api.js` and `/portal/client/activate` and `/portal/client/login` already existed server-side, but nothing in the application called either, so the portal had no entrance other than a one-time email link.
- Let a password-backed session outlive the invitation. The portal cookie lasted 7 days (`PORTAL_TOKEN_EXPIRE_MINUTES`) and the invite 14 (`INVITE_TTL_DAYS`), after which a client could not get back in at all without phoning the firm. `invite.revoked` remains the authorization control and magic-link sessions still expire with the link.
- Resolve the matter on login: an account with one matter goes straight in, and one spanning several answers 409 with the choice list rather than 403.
