...
- Add `POST /api/platform/email/webhook/postmark`, which applies the relay's bounce and spam-complaint events. Postmark does not sign webhook payloads, so the endpoint is authenticated by a constant-time bearer secret comparison (`PLATFORM_EMAIL_WEBHOOK_SECRET`, length- and placeholder-checked at startup) rather than an HMAC over the body, and is allowlisted in `test_route_auth_coverage` with that reason. Deliveries are deduplicated on `(provider, event_id)` in `platform_email_webhook_events`, so a retry cannot re-suppress an address an operator has since released.
- Only permanent failures suppress. A transient bounce — full mailbox, greylisting, a temporary DNS failure at the recipient's host — is logged and dropped, because a momentary outage at someone's mail provider must not cost them account recovery. The lookup also fails open: if the suppression table is unreachable the message is sent, since one message to a dead address is cheaper than locking every user out of password reset.
...
- Add `docs/platform_email_setup.md`: why system email is a separate lane from matter correspondence (which still sends from the firm's own connected mailbox via `connected_mail.py`), why a transactional relay is preferred over an M365 app registration, SMTP AUTH client submission, or self-hosted Postfix, and the manual DNS and Microsoft 365 steps. The DNS section uses a custom Return-Path on a bounce subdomain so SPF stays evaluated against that subdomain and the root SPF record is never widened away from M365, while DKIM still aligns on the root and DMARC passes on DKIM alignment.
